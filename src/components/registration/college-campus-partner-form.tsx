...
import { CollegeCampusPartnerTermsContent } from "@/components/registration/college-campus-partner-terms-content";
import {
  collegeCampusPartnerApplicationSchema,
  type CollegeCampusPartnerApplicationInput,
...
export function CollegeCampusPartnerForm({ colleges }: { colleges: CollegeOption[] }) {
  ...
  } = useForm<CollegeCampusPartnerApplicationInput>({
    resolver: zodResolver(collegeCampusPartnerApplicationSchema),
    defaultValues: {
      collegeId: "",
      stream: "",
      name: "",
  ...
  // straight to the API route — a raw navigation has no way to show a
  // friendly error if the download fails.
  async function downloadCertificate(id: string) {
    setDownloading(true);
    setDownloadError(null);
    try {
      const res = await fetch(`/api/college-campus-partner/certificate/${id}/download`);
      if (!res.ok) {
      ...
      const filename = filenameMatch?.[1] ?? "YCC-College-Campus-Partner.pdf";
  ...
  async function onSubmit(values: CollegeCampusPartnerApplicationInput) {
    try {
      const res = await fetch("/api/college-campus-partner", {
        method: "POST",
    ...
    return (
      ...
      <Card className="border-slate-200/60 shadow-xl shadow-slate-200/40 rounded-[2rem] overflow-hidden bg-white text-center py-10">
        <CardHeader>
          ...
          <CardDescription>
            Your certificate is downloading now. Missed it? Use the button
            below, or re-download it any time from{" "}
            <Link
              href="/college-campus-partner/certificate"
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <Card className="border-slate-200/60 shadow-xl shadow-slate-200/40 rounded-[2rem] overflow-hidden bg-white">
        ...
        <CardContent className="space-y-5 px-6 sm:px-10 py-8">
          ...
          <Field label="Stream" error={errors.stream?.message}>
            <Input {...register("stream")} placeholder="e.g. B.Tech Computer Science" />
          </Field>
      ...
      <Dialog open={termsOpen} onOpenChange={setTermsOpen}>
        <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle>YCC College Campus Partner — Terms &amp; Conditions</DialogTitle>
          </DialogHeader>
          <CollegeCampusPartnerTermsContent />
        </DialogContent>
      </Dialog>
    </form>
  );
