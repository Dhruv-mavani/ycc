  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 flex flex-col items-end sm:bottom-6 sm:right-6">
      ...
      <div className="relative">
        ...
        >
          <div className={`absolute inset-0 flex items-center justify-center rounded-full overflow-hidden transition-all duration-500 ${open ? 'rotate-90 opacity-0 scale-50' : 'rotate-0 opacity-100 scale-100'}`}>
            <Image src="/yuvan/avatar.jpg" alt="YUVAN" width={64} height={64} className="size-full object-cover rounded-full" />
          </div>
          <div className={`absolute inset-0 flex items-center justify-center rounded-full transition-all duration-500 text-blue-500 bg-background/80 backdrop-blur-sm ${open ? 'rotate-0 opacity-100 scale-100' : '-rotate-90 opacity-0 scale-50'}`}>
            <X className="size-8" />
          </div>
        </Button>
      </div>
