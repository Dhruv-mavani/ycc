...
import { RegistrationTypeFilter } from "@/components/admin/registration-type-filter";
...
export default async function AdminFreeRegistrationsPage({
  searchParams,
}: {
  searchParams: Promise<{ type?: "school" | "individual" }>;
}) {
  const { type } = await searchParams;
  const selected = type === "individual" ? "individual" : "school";
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-4 px-4">
          ...
          <Button
            ...
            render={
              <a
                href={
                  selected === "school"
                    ? "/api/admin/export/school-registrations"
                    : "/api/admin/export/individual-free-registrations"
                }
              >
      ...
      {selected === "school" ? <SchoolRegistrationsTable /> : <IndividualRegistrationsTable />}
    </div>
  );
}

async function SchoolRegistrationsTable() {
...
async function IndividualRegistrationsTable() {
  const admin = createAdminClient();
  const { data: registrations } = await admin
