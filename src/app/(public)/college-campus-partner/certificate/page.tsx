...
import {
  collegeCampusPartnerCertificateLookupSchema,
  type CollegeCampusPartnerCertificateLookupInput,
} from "@/lib/validations/college-campus-partner";

export default function CollegeCampusPartnerCertificateLookupPage() {
  ...
  } = useForm<CollegeCampusPartnerCertificateLookupInput>({
    resolver: zodResolver(collegeCampusPartnerCertificateLookupSchema),
    defaultValues: { query: "" },
  });

  async function onSubmit(values: CollegeCampusPartnerCertificateLookupInput) {
    setFormError(null);
    try {
      const res = await fetch("/api/college-campus-partner/certificate/lookup", {
        method: "POST",
      ...
      window.location.assign(
        `/api/college-campus-partner/certificate/${data.applicationId}/download`,
      );
    } catch {
        ...
        <Card className="border-slate-200/60 shadow-xl shadow-slate-200/40 rounded-[2rem] overflow-hidden bg-white">
          <CardHeader className="bg-slate-50/50 border-b border-slate-100 pb-6 px-6 sm:px-10 pt-8">
            ...
            <CardDescription className="text-base mt-2 text-slate-600">
              For YCC College Campus Partners. Enter the mobile number you
              applied with, or the code from your certificate.
            </CardDescription>
          </CardHeader>
          <CardContent className="px-6 sm:px-10 py-8">
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              <div className="space-y-1.5">
                <Label>Mobile number or code</Label>
                <Input
                  {...register("query", {
                    onChange: () => setFormError(null),
                  })}
                  placeholder="9876543210 or TEST7771"
